...
export class RelatorioDevolucao {
  ...
  async abrir() {
    ...
    await aguardarElementoComReload(this.page, '#opt-relatorio-devolucao');
    
    await navegarPara("#opt-relatorio-devolucao", "relatorio-devolucao", this.page);
  }
  ...
  async verificarGraficoExiste(): Promise<boolean> {
    try {
      await this.page.waitForSelector('#grafico-gevolucao', { timeout: 10000 });
      
      const canvas = await this.page.locator('#grafico-gevolucao');
