...
export class GestorLocacao {
  ...
  async registrarLocacao(dadosLocacao: DadosLocacao): Promise<RespostaLocacao> {
    ...
    if (!todosValidos) {
      console.error("Alguns equipamentos são inválidos:", 
    ...
    console.log("Equipamentos a enviar:", 
      dadosLocacao.itens.map(i => `${i.equipamento.descricao} (${i.equipamento.codigo})`));
