...
import { GestorCliente } from "../src/cliente/gestor-cliente";
...
describe("GestorCliente", () => {
  ...
  it("Deve lançar erro com status 404", async () => {
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      ok: false,
      status: 404,
      json: async () => ({}),
    ...
    await expect(gestor.obterClientes(999)).rejects.toBeInstanceOf(ErroDominio);
  });
});
